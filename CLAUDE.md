...
- **Whole-issue PDF galleys duplicate every article's content.** Any article content correction must also be applied to the issue PDF (replace the file at `issues/<id>/public/` in the container; corrected versions belong in the `backfill/private/output/<vol.iss>/issue-galley.pdf` presave slot so pipe9 reuses them). See `docs/ojs-issues-log.md` #37.
