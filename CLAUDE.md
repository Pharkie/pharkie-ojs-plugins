...
- **🛑 Never run `pipe9b` with `--issue`.** The `--issue` flag scopes the INSERT but *not* the DELETE, which is unconditional (`DELETE FROM citation_settings WHERE setting_name='crossref::doi'`). `pipe9b --target live --issue 37.2 --confirm` therefore wipes every reference DOI in the journal and writes back only that issue's — 6,945 rows down to 146, live, in one command (2026-08-10). It reports "Done. 146 DOIs written" and looks like a success. Always run it unscoped; a full run is seconds and rewrites everything from JATS. Verify after: `SELECT COUNT(*), COUNT(DISTINCT setting_value) FROM citation_settings WHERE setting_name='crossref::doi'` should be ~6,945 / ~4,506.
