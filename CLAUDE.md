...
- [`docs/similar-articles-plugin.md`](docs/similar-articles-plugin.md) — Similar Articles plugin: cached-similarity "Related articles" sidebar (replaces stock recommendBySimilarity)
...
- **Similar articles sidebar** uses the custom `similarArticles` plugin (`plugins/similar-articles/`), not the stock `recommendBySimilarity` (which is disabled — it collapses on this thematically narrow corpus; see `docs/ojs-issues-log.md` #26). Rebuild the cache with `sudo python3 scripts/ojs/build_similar_articles.py --target=dev|live` (sklearn TF-IDF, takes <2s). Nightly cron runs it against live automatically. Full docs: `docs/similar-articles-plugin.md`.
