...
- **Whole-issue PDF galleys duplicate every article's content.** Any article content correction must also be applied to the issue PDF — and it can exist on disk under multiple `issues/<id>/public/` dirs from stale imports; replace every copy. See `docs/ojs-issues-log.md` #37.
