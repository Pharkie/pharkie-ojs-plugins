...
**Galley file uploads take effect immediately.** In the "Upload a File Ready for Publication" wizard, the file replaces what readers download the moment it uploads in step 1 — the Continue/Confirm steps only edit metadata, and **Cancel does not undo the replacement** (`docs/ojs-issues-log.md` #36). Only drop a file there if it's the final file. If the wrong file goes up, ask the site admin — the previous file is recoverable.
