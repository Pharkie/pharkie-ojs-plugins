...
| 38 | pipe4b cache hits reused stale ref_id — DOI on wrong reference | Tooling bug | Cache rows re-stamped with current ref_id |
...
- **Stale import IDs (fixed 2026-07-13):** pipe8's issue remap covered `issues`/`issue_settings`/`issue_galleys` but not `issue_files.issue_id`, and never moved the on-disk `issues/<id>/` directory — so every reimport left the galley's file row pointing at the import-time issue ID and accumulated another stale directory. Found at scale: all 68 galleys crossed on both dev and live; 2,897 orphan `issue_files` rows and 3.1GB of stale copies on dev (861 files), 340 rows / 509MB on live. Cleaned up (galley row = source of truth: file moved to the galley's issue dir, row re-pointed, orphans and unreferenced files deleted; tables dumped first). pipe8 now remaps `issue_files` and moves the directory, so this class is closed. If it ever recurs, sweep with: `find /var/www/files/journals/1/issues -name "vol-X-iss-Y.pdf"`.
- **Pipeline inputs stay unredacted:** the archival source (`backfill/input/`) is deliberately left untouched, so a re-run of the split pipeline would regenerate unredacted split PDFs for corrected articles. Check `_manual_html`/redaction notes in the volume's toc.json before re-splitting. For the *issue* PDF, a corrected version placed at `backfill/private/output/<vol.iss>/issue-galley.pdf` takes precedence in `pipe9_issue_galleys.sh`, so future galley uploads use it automatically (33.2's re-typeset PDF lives there).

### 38. pipe4b cache hits reused the cached row's stale ref_id — DOI hangs on the wrong reference after refs shift [tooling-bug]

`doi_matches.json` lookups are keyed on reference *text* (by design, resilient to reordering), but each cached row also stores the `ref_id` it had when first matched — and on a cache hit `process_article` appended the cached row **verbatim**. `write_dois_to_jats` keys on `ref_id`, so after a reference was deleted (renumbering everything after it), the cached row's stale id pointed at whichever reference now occupied that position. Concretely: removing an orphaned reference from a 33.2 book review shifted Upton (ref5→ref4), and Upton's own-journal DOI got written onto the reference now sitting at ref5 (Van der Kolk). The verbose log looked correct throughout — it prints the *current* ref_id while appending the stale one — and the misplaced pub-id then self-validates on later runs (the ref "already has a DOI").

- **Impact:** wrong DOI served in the JATS galley and written to `citation_settings` by pipe9b for ~3 hours until caught. Only bites when a reference is added/removed between cache creation and a pipeline rerun — the first time that had ever happened.
- **Fix:** cache hits are re-stamped with the current `ref_id`/text before appending (`dict(cached, ref_id=ref_id, text=text)`). Caches self-heal on the next cache-mode run (the run rewrites `doi_matches.json` from results).
- **Caught by:** the successor-system import validating its hand-checked JATS against the regenerated bytes. Worth remembering that the OJS-side symptom (a `10.65828/…` own-prefix DOI on a reference to an external book) is cheap to grep for after any rerun that changes a ref list.

### 39. Two heavy jobs at once took the whole box down [ops]

Running `pipe7_import.sh --force` on a 35MB issue XML at the same time as a
CI-triggered Harbour deploy (Docker + Astro build) exhausted the box. Every
vhost went down — OJS, the new site, Harbour, Outline, Umami — and Better Stack
alerted on all of them. CPU sat pinned at 200% (both vCPUs) with near-zero disk
I/O, which is why `ssh` failed *during banner exchange*: no process could get a
slice, sshd included.

The box is **2 vCPUs / 3.8 GB** running thirteen containers (OJS + MariaDB, WP +
...
- **Recovery:** confirmed via `hcloud server metrics` that CPU was flat and disk
  ...
  *not* `reset` — a hard power cycle with MariaDB and Postgres running is how a
  10-minute outage becomes a data-loss incident). Back in ~90 seconds, all
  containers healthy, no data lost.
- **Diagnosis without SSH:** `hcloud server metrics sea-live --type cpu|disk`
  works when the box won't answer. Flat CPU + no disk I/O = spinning process,
  safe to reboot. Rising disk I/O = something is writing, wait it out.
- **Rule:** never start a pipeline import when CI might deploy. Check
  `gh run list --limit 1` first, and don't push to either repo while an import
  is running. Watch `free -m` during the import if in any doubt.
- **Also bitten:** an earlier CI deploy failed with `client_loop: send
  disconnect: Broken pipe`. That was not a deploy bug — it was the box already
  going under.

### 40. pipe4 run standalone silently moved citations between articles [tooling-bug]

`pipe4_extract_citations.py` moves references from the JATS *body* into back
matter. Run a second time, the body no longer holds them, so it re-derives the
back matter from whatever is left. On 37.2 that cost article 13 four of its ten
references (it has three reference lists — main plus two appendices — and only
the tail one is extracted).

What made it dangerous: the issue total stayed at **335**, because article 16
gained exactly the four that article 13 lost. Checking the total said "no
change" while two articles were wrong.

- **Rule:** `pipe4` follows `pipe3`, always. `pipe3` regenerates the JATS with
  references back in the body, which is the state `pipe4` expects. Never run
  `pipe4 --extract` on its own against already-extracted JATS. (CLAUDE.md already
