...
**Test webhook removed (2026-03-26):** Stripe test mode webhook was pointing at the live OJS URL, causing failed delivery emails (36 failures since 2026-03-23). Removed because: (1) there's no staging OJS instance for it to hit, (2) the live plugin runs in live mode (`testMode=0`) so it rejects test-signed events, (3) making the plugin accept both secrets would allow test events to trigger real actions on live. Live webhook kept. A Stripe Sandbox has been created for future testing — use it with its own isolated webhook pointing at a dev/staging OJS instance with `testMode=1`.
