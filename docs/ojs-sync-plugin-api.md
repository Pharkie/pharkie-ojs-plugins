...
**Username mismatch risk:** WP usernames commonly contain characters stripped by sanitization (dots, hyphens, underscores, spaces, `@` — especially email-as-username accounts). If those users typed their exact WP login into OJS, the unsanitized input wouldn't match the sanitized stored username and login would fail. In practice this is low-risk: email-as-username logins (e.g. `user@example.com`) are detected as emails and matched correctly, and the UI steers everyone toward email login. The remaining edge cases are WP logins with dots/hyphens/underscores that don't look like emails (e.g. `aki.olver` stored as `akiolver`) — those users must use their email to log in.
